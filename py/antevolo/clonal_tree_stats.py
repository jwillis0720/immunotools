...
#            fh.write(src_id + ' -> ' + dst_id + '\n')
        fh.write('Src_ID\tDst_ID\tEdge_type\tNum_reversed_v\tNum_added_v\tNum_reversed_j\tNum_added_j\tnum_CDR3\n')
        for e in self.directed_clonal_tree.EdgeIter():
            edge_stats = self.directed_clonal_tree.GetStatsByEdge(e)
            src_id = self.directed_clonal_tree.GetSequenceByVertex(e[0]).id
            dst_id = self.directed_clonal_tree.GetSequenceByVertex(e[1]).id
            fh.write(src_id + '\t' + dst_id + '\t' + edge_stats.edge_type.name + '\t' + str(edge_stats.NumReversedV()) + '\t' + str(edge_stats.NumAddedV()) + '\t' + str(edge_stats.NumReversedJ()) + '\t' + str(edge_stats.NumAddedJ()) + '\t' + str(edge_stats.NumCDR3SHMs()) + '\n')
        fh.close()
